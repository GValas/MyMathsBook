...
\begin{mdframed}[style=proofbar]
  ...
  comme $\gcd(p,q) = 1$, le théorème de Bézout fournit $u,v \in \mathbb{Z}$ avec
  ...
  \smallskip\noindent
  Enfin $\{n\alpha\} = n\alpha - \lfloor n\alpha\rfloor$ appartient à $G \cap [0,1[$
  (car $n\alpha \in G$ et $\lfloor n\alpha\rfloor \in \mathbb{Z} \subseteq G$), et
  lorsque $n$ décrit $\mathbb{Z}$ il parcourt même tout $G \cap [0,1[$ ;
  la densité de $G$ dans $\mathbb{R}$ équivaut donc à celle de $(\{n\alpha\})$ dans $[0,1]$.
  \hfill$\blacksquare$
\end{mdframed}
