...
\begin{mdframed}[style=proofbar]
  ...
  Soit $p = \mathrm{car}(K) > 0$. D'abord $p \geq 2$, car $p = 1$ signifierait
  $1 = 0$, exclu par la définition d'un corps.
...
\begin{significationintuitive}
  Le sous-anneau engendré par $1$ est l'image du morphisme d'anneaux
  $\mathbb{Z} \to K,\; n \mapsto n \cdot 1$, dont le noyau est $\mathrm{car}(K)\,\mathbb{Z}$.
  Ce sous-anneau est donc isomorphe à $\mathbb{Z}$ (si $\mathrm{car}(K) = 0$) ou à
  $\mathbb{Z}/p\mathbb{Z}$ (si $\mathrm{car}(K) = p$). Le plus petit sous-corps de $K$ est
  le corps des fractions de ce sous-anneau, d'où $\mathbb{Q}$ ou $\mathbb{F}_p$
  respectivement.
...
\begin{ideepreuve}
  Si $(e_i)_{i \in I}$ est une base de $M$ sur $L$ et $(f_j)_{j \in J}$ une base de
  $L$ sur $K$, alors la famille des produits $(e_i f_j)_{(i,j) \in I \times J}$ forme
  une base de $M$ sur $K$ : tout élément de $M$ s'écrit $\sum_i \lambda_i e_i$ avec
  $\lambda_i \in L$, puis chaque $\lambda_i = \sum_j \mu_{ij} f_j$ avec
  $\mu_{ij} \in K$. On obtient $[M:K] = |I|\cdot|J| = [M:L]\cdot[L:K]$, l'égalité
  valant aussi dans le cas infini avec la convention des cardinaux.
\end{ideepreuve}
...
\begin{mdframed}[style=proofbar]
  ...
  est un morphisme de noyau $(\mu_\alpha)$, d'image $K[\alpha]$ ; comme
  $K[\alpha] \cong K[X]/(\mu_\alpha)$ est un corps, on a $K[\alpha] = K(\alpha)$.
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{tcolorbox}[thmbox]
  ...
  \begin{enumerate}
    ...
    \item Pour tout $p$ premier et $n \geq 1$, il existe, à isomorphisme près,
          un unique corps à $p^n$ éléments, noté $\mathbb{F}_q$ ou
          $\mathrm{GF}(q)$ (corps de Galois).
...
\begin{mdframed}[style=proofbar]
  ...
  \textit{Existence et unicité (point 2).}
  Elles reposent sur la théorie des corps de décomposition et sont admises ici.
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item (Corps gauche des quaternions) Soit
          $\mathbb{H} = \{a + bi + cj + dk \mid a,b,c,d \in \mathbb{R}\}$
          avec $i^2 = j^2 = k^2 = ijk = -1$. Vérifier que $ij = k = -ji$, puis
          que tout $q \neq 0$ est inversible, d'inverse $\bar q/N(q)$ où
          $\bar q = a - bi - cj - dk$ et $N(q) = q\bar q = a^2+b^2+c^2+d^2$.
          Ainsi $\mathbb{H}$ est un corps gauche non commutatif — nécessairement
          infini, d'après le théorème de Wedderburn (\emph{tout corps gauche
          fini est commutatif}, admis).
  \end{enumerate}
\end{mdframed}
