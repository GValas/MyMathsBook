...
\lecontitle{Hyperplans de \texorpdfstring{$\mathcal{M}_n(\mathbb{R})$}{Mn(R)}}{Algèbre linéaire — Rencontre avec $\mathrm{GL}_n(\mathbb{R})$ et $\mathrm{O}_n(\mathbb{R})$}

%=============================================================
%  § 1 — PRÉLIMINAIRE : FORMES LINÉAIRES ET TRACE
%=============================================================
...
\begin{mdframed}[style=proofbar]
  ...
  pour toute $M$, on choisit $M = E_{ji}$ (matrice élémentaire). Le produit
  $A E_{ji}$ ne conserve que la $j$-ème colonne de $A$, reportée en $i$-ème
  colonne, de sorte que $\mathrm{tr}(AE_{ji}) = A_{ij}$. Cette quantité étant
  nulle pour tous $i,j$, on a $A = 0$. \hfill$\blacksquare$
\end{mdframed}
...
\rubriq{Idée}
On écrit $H = \ker\bigl(Q\mapsto\mathrm{tr}(AQ)\bigr)$. La
...
cherche un zéro. On considère alors le \emph{chemin continu} de rotations $\theta \mapsto R(\theta)$
...
\begin{mdframed}[style=proofbar]
  ...
  \]
  car alors $Q = V^T R\, U^T \in H \cap \mathrm{O}_n(\mathbb{R})$.
  ...
  $\rho(\theta) = \bigl(\begin{smallmatrix}\cos\theta & -\sin\theta\\ \sin\theta & \cos\theta\end{smallmatrix}\bigr)$.
  \begin{itemize}
...
$M := \int Q\,\mathrm{d}Q$ satisfait, par invariance de la mesure de Haar par
translation à gauche, $RM = \int RQ\,\mathrm{d}Q = \int Q\,\mathrm{d}Q = M$ pour
tout $R\in\mathrm{SO}_n$ ; chaque colonne de $M$ est donc fixée par toutes les
rotations, et le seul vecteur ainsi fixé est nul ($n\geq 2$ : une rotation dans
un plan contenant un vecteur non nul le déplace). Dès lors
\[
...
%=============================================================
%  § 4 — EXEMPLES, REMARQUES, EXERCICES
%=============================================================
...
\begin{mdframed}[style=exbar]
  ...
  trace nulle et inversible, y vit : $C \in H \cap \mathrm{GL}_n$. Mieux :
  matrice de permutation, $C$ est orthogonale pour tout $n$, donc
  $C \in H \cap \mathrm{O}_n$ dès que $n \geq 2$ (pour $n=2$,
  $C = \bigl(\begin{smallmatrix}0&1\\1&0\end{smallmatrix}\bigr)$) — pour cet
  hyperplan particulier, la construction de la Partie~II est donc superflue.

  \smallskip\noindent
  \textbf{2. Cas $n=2$, $A = I_2$, version orthogonale fine.}\enspace%
  La rotation $\rho(\theta)$ a pour trace $2\cos\theta$, nulle pour
  $\theta = \pi/2$ : $\rho(\pi/2) = \bigl(\begin{smallmatrix}0&-1\\1&0\end{smallmatrix}\bigr)
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item \textbf{(Moyennisation)} Justifier $\int_{\mathrm{SO}_n} Q\,\mathrm{d}Q = 0$
          pour $n\geq 2$ et en redéduire la Partie II sans recourir à la SVD.
          (On intègre et l'on conclut sur $\mathrm{SO}_n$, qui est connexe,
          et non sur $\mathrm{O}_n$.)

    \item Le résultat de la Partie II subsiste-t-il en remplaçant $\mathrm{O}_n(\mathbb{R})$
          par $\mathrm{SO}_n(\mathbb{R})$~? (\textit{Oui, mais le chemin de la
          Partie II ne suffit pas tel quel~:} $R(\theta)$ reste dans
          $\mathrm{SO}_n$, mais $\det\bigl(V^T R(\theta)\, U^T\bigr)
          = \det(UV)$ peut valoir $-1$, p.~ex.\ si $A$ est une réflexion~;
          utiliser la moyennisation sur $\mathrm{SO}_n$ de l'exercice
          précédent, ou corriger le signe dans la SVD.) Et par
          $\mathrm{O}_n \setminus \mathrm{SO}_n$~?
  \end{enumerate}
\end{mdframed}
