...
\begin{tcolorbox}[thmbox]
  ...
  \]
  \textit{Remarque.} Le résultat est valable sur un corps quelconque
  $\mathbb{K}$ : la preuve par la comatrice ci-dessous est purement
...
\begin{mdframed}[style=proofbar]
  ...
  diagonalisables sont denses dans $\mathcal{M}_n(\mathbb{C})$ (une matrice à
  valeurs propres deux à deux distinctes est diagonalisable, et de telles
  matrices sont denses car le discriminant de $\chi_A$ ne s'annule pas
  génériquement). Comme $A\mapsto\chi_A(A)$ est continue (polynomiale en les
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item (Décomposition de Dunford) On suppose $\chi_A$ scindé,
          $\chi_A = \prod_i {(X-\lambda_i)}^{n_i}$ (ce qui est automatique
          sur $\mathbb{C}$). En appliquant le lemme des noyaux à cette
          factorisation, montrer que $E$ se décompose en somme directe de
