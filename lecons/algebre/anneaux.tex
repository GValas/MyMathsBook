...
\begin{tcolorbox}[thmbox]
  ...
  \end{enumerate}
  Si $A$ est commutatif, les deux conditions d'absorption $ax \in I$ et
  $xa \in I$ coïncident (idéal à gauche $=$ à droite $=$ bilatère).
  Si $A$ est commutatif, l'idéal \emph{engendré} par $a \in A$ est
  $(a) = aA = \{ra \mid r \in A\}$.
...
\begin{significationintuitive}
  Un idéal est à un anneau ce qu'un sous-groupe distingué est à un groupe : c'est
  exactement le type de partie par laquelle on peut quotienter pour que la
  \emph{multiplication} passe au quotient. La condition d'absorption ($ax, xa \in I$
  pour tout $a \in A$, $x \in I$) garantit que la classe d'un produit ne dépend que
  des classes des facteurs ; c'est pourquoi ce sont les idéaux — et non les simples
  sous-anneaux — qui jouent le rôle de noyaux des morphismes d'anneaux.
...
\begin{mdframed}[style=proofbar]
  ...
  surjective sur $\mathrm{Im}(\varphi)$, et c'est un morphisme.
  $\blacksquare$
\end{mdframed}
...
\begin{tcolorbox}[thmbox]
  ...
  \begin{itemize}
    \item \textbf{Euclidien}\index{anneau euclidien} : muni d'un stathme (division euclidienne).
          ...
          (Tout corps est euclidien, avec stathme constant $\varphi \equiv 0$ ;
          la flèche $\text{Corps}\subsetneq\text{Euclidien}$ signifie que la
          classe des euclidiens est \emph{strictement plus grande}.)
...
\begin{ideepreuve}
  Les inclusions de la chaîne s'obtiennent ainsi.
  \emph{Euclidien $\Rightarrow$ principal.} Soit $I \neq \{0\}$ un idéal et
  $a \in I \setminus \{0\}$ de stathme $\varphi(a)$ minimal. Pour $b \in I$, la division
  euclidienne donne $b = qa + r$ avec $r = 0$ ou $\varphi(r) < \varphi(a)$ ; mais
  $r = b - qa \in I$, donc la minimalité de $\varphi(a)$ impose $r = 0$, d'où $b \in (a)$
  et $I = (a)$.
  \emph{Principal $\Rightarrow$ factoriel.} Dans un anneau principal, toute suite
  croissante d'idéaux stationne (il est noethérien), ce qui garantit l'existence d'une
  décomposition en irréductibles ; et tout élément irréductible y engendre un idéal
  maximal, donc est premier, ce qui en assure l'unicité.
\end{ideepreuve}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    \item Montrer qu'un anneau commutatif intègre $A$ est un corps si et
          seulement si $\{0\}$ est son seul idéal premier (c'est-à-dire si et
          seulement si $\{0\}$ est maximal).
    ...
    \item (Lemme de Nakayama) Soit $A$ commutatif local (unique idéal maximal $\mathfrak{m}$)
          et $M$ un $A$-module de type fini. Montrer que si $M = \mathfrak{m}M$, alors $M = 0$.
  \end{enumerate}
\end{mdframed}
