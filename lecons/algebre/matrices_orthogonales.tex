...
\begin{ideepreuve}
  ...
  propres vaut $\det Q=1$ et que les éventuelles valeurs propres non réelles forment
  une paire conjuguée $e^{i\theta},e^{-i\theta}$ de produit $1$, la valeur propre
  réelle vaut nécessairement $+1$. Il existe donc un vecteur unitaire $u$ avec $Qu=u$,
...
\begin{mdframed}[style=proofbar]
  ...
  vecteurs propres $w_k,\overline{w_k}$, on regroupe ces deux valeurs propres : la
  partie réelle et la partie imaginaire $u_k=\mathrm{Re}(w_k)$, $v_k=\mathrm{Im}(w_k)$
  engendrent un plan réel stable par $Q$, sur lequel $Q$ agit comme la rotation
  $R_{\theta_k}$. En normalisant les familles $(u_k,v_k)$ ainsi obtenues, on construit
  une base orthonormale \emph{réelle} de $\mathbb{R}^n$, c'est-à-dire une matrice
  $P\in\mathrm{O}(n)$, dans laquelle $P^TQP$ est bloc-diagonale, à blocs $(\pm 1)$ et
  $R_{\theta_k}$. $\blacksquare$
\end{mdframed}
