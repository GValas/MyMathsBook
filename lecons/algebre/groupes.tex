...
\begin{tcolorbox}[thmbox]
  ...
  \begin{enumerate}
    \item[\textbf{G1.}] \textbf{Associativité :} $\forall a,b,c \in G,\; (a\cdot b)\cdot c = a\cdot(b\cdot c)$.
    \item[\textbf{G2.}] \textbf{Élément neutre :} $\exists e \in G,\; \forall a \in G,\; e\cdot a = a\cdot e = a$.
...
\begin{mdframed}[style=exbar]
  ...
  $(\mathbb{U},\times)$ (complexes de module $1$) sont abéliens.
  ...
  L'ensemble des matrices inversibles de $\mathcal{M}_n(\mathbb{K})$, muni du produit matriciel.
...
\begin{mdframed}[style=proofbar]
  ...
  $H$ contient un entier $> 0$ (car si $a \in H$, $a\neq 0$, alors comme $a$ et $-a$ appartiennent à $H$, on a $|a|\in H$).
...
\begin{mdframed}[style=proofbar]
  ...
  Donc $|G| = |H| \cdot |\{$classes$\}| = |H| \cdot [G:H]$.

  \textit{Corollaire.} Appliqué au sous-groupe $\langle a\rangle$, dont le cardinal
  vaut précisément l'ordre de $a$, le théorème montre que cet ordre divise $|G|$ ;
  écrivant $|G| = \mathrm{ord}(a)\cdot m$, on obtient $a^{|G|} = (a^{\mathrm{ord}(a)})^m = e$.
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Montrer que tout groupe d'ordre $p^2$ ($p$ premier) est abélien.
          (\textit{Indication :} considérer le centre $Z(G)$.)
