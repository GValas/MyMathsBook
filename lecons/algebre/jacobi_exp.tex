...
\begin{mdframed}[style=proofbar]
  ...
  \[
    W'(t) = \sum_{j=1}^n \det(x_1,\ldots,x_j',\ldots,x_n)
           = \sum_{j=1}^n \det(x_1,\ldots,A(t)x_j,\ldots,x_n),
  \]
  où, dans le $j$-ème terme, seule la $j$-ème colonne est dérivée.
  Décomposons $A(t)x_j$ sur la base canonique~: en notant
  $A(t)=(a_{k\ell}(t))$, on a $A(t)x_j = \sum_{k=1}^n a_{kj}(t)\,x_k$, d'où
  \[
    \det(x_1,\ldots,A(t)x_j,\ldots,x_n)
      = \sum_{k=1}^n a_{kj}(t)\,
        \det(x_1,\ldots,\underbrace{x_k}_{\text{place }j},\ldots,x_n).
  \]
  Pour $k \neq j$, le déterminant possède deux colonnes égales
  (la colonne $x_k$ figure déjà en position $k$ et est répétée en position $j$)~:
  il est donc nul. Seul le terme $k=j$ survit, et il vaut
  $a_{jj}(t)\,\det(x_1,\ldots,x_n) = a_{jj}(t)\,W(t)$. En sommant sur $j$~:
  \[
    W'(t) = \sum_{j=1}^n a_{jj}(t)\,W(t) = \mathrm{tr}(A(t))\,W(t).
  \]
