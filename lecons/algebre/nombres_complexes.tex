...
\lecontitle{Nombres complexes}{Algèbre — Arithmétique de \texorpdfstring{$\mathbb{C}$}{C}}
...
\begin{tcolorbox}[thmbox]
  ...
  \index{nombres complexes!forme trigonométrique}
  Tout $z \neq 0$ s'écrit
  \[
...
\begin{tcolorbox}[thmbox]
  ...
  Multiplier par $r > 0$ est une homothétie de rapport $r$.
\end{tcolorbox}
...
\begin{tcolorbox}[thmbox]
  ...
  \]
  Pour $n \geq 3$, ces solutions sont les sommets d'un polygone régulier à
  $n$ côtés inscrit dans le cercle de centre $O$ et de rayon $\rho^{1/n}$,
  deux sommets consécutifs étant séparés d'un angle $2\pi/n$.
  ...
  pour $j \not\equiv 0 \pmod{n}$, et $\displaystyle\sum_{k=0}^{n-1} \omega_k^j = n$
  pour $j \equiv 0 \pmod{n}$ (relation d'orthogonalité).
\end{tcolorbox}
...
\begin{mdframed}[style=proofbar]
  \textit{Idée de preuve (cocyclicité).} L'argument du quotient
  $\dfrac{z_3-z_1}{z_3-z_2}$ mesure l'angle $\widehat{M_1 M_3 M_2}$ ; par le
  théorème de l'angle inscrit\index{angle inscrit (théorème de l')}, les quatre
  points sont sur un même cercle (ou une même droite) si et seulement si les
  angles vus depuis $z_3$ et $z_4$ sont égaux modulo $\pi$, c'est-à-dire si le
  birapport est réel. \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=exbar]
  ...
  On factorise numérateur et dénominateur par l'angle moitié
  ($1 - e^{i\alpha} = -2i\sin(\alpha/2)\,e^{i\alpha/2}$) pour obtenir la
  formule fermée :
  \[
...
\begin{mdframed}[style=cexbar]
  \textbf{L'argument principal n'est pas additif.}\enspace%
  ...
  (avec la détermination principale $\arg(-1) = \pi$).
  ...
  $\lambda \geq 0$ (ou $z = 0$). Pour $z$ et $w$ non nuls, cela équivaut à
  $\arg z \equiv \arg w \pmod{2\pi}$~; mais la condition sur les arguments,
  seule, est incomplète : elle ne couvre pas le cas où l'un des deux est nul.
\end{mdframed}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Soit $P = X^n + a_{n-1}X^{n-1} + \cdots + a_0 \in \mathbb{R}[X]$.
    ...
    \item (Inégalité de Ptolémée) Montrer que pour tous $z_1, z_2, z_3 \in \mathbb{C}$ :
          ...
          \]
          (\textit{Indication :} vérifier l'identité algébrique
          $z_2(z_1 - z_3) = z_3(z_1 - z_2) + z_1(z_2 - z_3)$, puis appliquer
          l'inégalité triangulaire et passer aux modules.)
  \end{enumerate}
\end{mdframed}
