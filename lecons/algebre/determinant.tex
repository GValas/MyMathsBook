...
\begin{tcolorbox}[thmbox]
  ...
  qui soit :}
  \begin{enumerate}
    \item \textit{\textbf{Multilinéaire} par rapport aux colonnes (ou aux lignes) :
    ...
    \item \textit{\textbf{Alternée} : $\det(C_1,\ldots,C_n)=0$ dès que deux colonnes
          sont égales (équivalent : échanger deux colonnes change le signe) ;}
    \item \textbf{Normalisée} : $\det(I_n) = 1$.
  \end{enumerate}
...
\begin{mdframed}[style=proofbar]
  ...
  Par unicité à constante multiplicative près des formes multilinéaires alternées,
  $\varphi(B) = \varphi(I_n)\,\det(B) = \det(A)\det(B)$. \hfill$\blacksquare$
\end{mdframed}
...
La \emph{comatrice} (ou matrice adjointe classique) est la transposée de la
matrice des cofacteurs\index{comatrice} :
\[
...
\begin{mdframed}[style=exbar]
  ...
  \index{déterminant de Vandermonde}
  ...
  associée a pour polynôme caractéristique $(-1)^n P(X)$.
  Le déterminant de $XI - C_P$ se calcule par développement de Laplace
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Montrer que pour $A,B \in \mathcal{M}_n(\mathbb{K})$ et $n\geq 2$,
          $\det(A+B) = \det(A) + \det(B)$ n'est en général pas vrai,
          mais que c'est vrai si $\mathrm{rang}(A) \leq 1$ et $B = I_n$.
