...
\begin{mdframed}[style=proofbar]
  ...
  \textit{Claim : $(f(e_{p+1}),\ldots,f(e_n))$ est une base de $\mathrm{Im}(f)$.}
  ...
  \smallskip
  \textit{Famille libre.}
  Supposons $\sum_{j=p+1}^n \mu_j f(e_j) = 0$, i.e.\ $f\!\left(\sum_{j=p+1}^n \mu_j e_j\right) = 0$.
  Alors $\sum_{j=p+1}^n \mu_j e_j \in \ker f = \mathrm{Vect}(e_1,\ldots,e_p)$, donc
  \[
  ...
  $\dim(\mathrm{Im}\,f) = n - p$, et $p + (n-p) = n$. \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=proofbar]
  ...
  $\Leftrightarrow$ $\mathrm{rg}(A|b) = \mathrm{rg}(A)$ (théorème de Rouché-Fontené\index{Rouché-Fontené (théorème de)}).
...
\begin{mdframed}[style=exbar]
  ...
  (infiniment de solutions si $r < n$, unique si $r = n$).
...
\begin{mdframed}[style=cexbar]
  ...
  ($(1,0,0,\ldots)\notin\mathrm{Im}\,f$). Nullité $+$ rang $= 0 + \infty \neq \infty$.
