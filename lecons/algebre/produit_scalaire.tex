...
\begin{tcolorbox}[thmbox]
  ...
  \medskip
  \textbf{Théorème (Pythagore).}\enspace%
  \index{theoreme de Pythagore@théorème de Pythagore}
  $x \perp y$ si et seulement si $\|x+y\|^2 = \|x\|^2 + \|y\|^2$.
  Plus généralement, si $x_1, \ldots, x_p$ sont deux à deux orthogonaux :
  \[
    \|x_1 + \cdots + x_p\|^2 = \|x_1\|^2 + \cdots + \|x_p\|^2.
  \]
...
\begin{mdframed}[style=proofbar]
  \textit{Pythagore.} On développe :
  $\|x+y\|^2 = \|x\|^2 + 2\langle x,y\rangle + \|y\|^2$, d'où l'équivalence.
  Le cas général s'obtient de même : les doubles produits
  $2\langle x_i,x_j\rangle$, $i < j$, sont tous nuls.

  \smallskip
  \textit{Supplémentaire orthogonal.}
  ...
  Enfin, l'inclusion $F \subset (F^\perp)^\perp$ est immédiate, et
  $\dim (F^\perp)^\perp = \dim E - \dim F^\perp = \dim F$ donne l'égalité.
  $\blacksquare$
\end{mdframed}
...
\begin{tcolorbox}[thmbox]
  ...
  \]
  Ce minimum est appelé \emph{distance de $x$ à $F$}%
  \index{distance à un sous-espace} et noté
  $\mathrm{dist}(x,F) = \|x - p_F(x)\|$.
\end{tcolorbox}

\begin{mdframed}[style=proofbar]
  Pour $y \in F$ : $\|x-y\|^2 = \|x - p_F(x) + p_F(x) - y\|^2$.
  Or $x - p_F(x) \in F^\perp$ et $p_F(x) - y \in F$ sont orthogonaux, donc,
...
\begin{mdframed}[style=cexbar]
  ...
  \neq E$. L'égalité $(F^\perp)^\perp = F$ tombe aussi en défaut : on n'a en
  général que $F \subset (F^\perp)^\perp$ (dans un Hilbert,
  $(F^\perp)^\perp = \overline{F}$, l'adhérence de $F$). Dans un préhilbertien
  non complet, le résultat échoue de surcroît même pour certains sous-espaces
  fermés.
\end{mdframed}
