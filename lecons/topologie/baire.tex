...
recouvrir par une infinité dénombrable d'ensembles « minces ». La seconde
formulation se déduit de la première par passage au complémentaire.
...
\rubriq{De la première à la seconde formulation}
...
\begin{mdframed}[style=exbar]
  ...
  \[
    D_n = \Bigl\{f \in \mathcal{C}([0,1]) : \exists\, x_0\in[0,1],\;
    \sup_{\substack{h\neq 0\\ x_0+h\in[0,1]}}
    \frac{\bigl|f(x_0+h)-f(x_0)\bigr|}{|h|} \leq n\Bigr\}
  \]
  est fermé (si $f_k \to f$ uniformément avec des points témoins $x_0^{(k)}$,
  on extrait une sous-suite convergente dans le compact $[0,1]$ et on passe à
  la limite) et d'intérieur vide (toute $f$ s'approche uniformément par des
  fonctions affines par morceaux dont chaque pente excède $n$ en valeur
  absolue, lesquelles échappent à $D_n$).
  Par Baire, $\bigcup_{n\geq 1} D_n \neq \mathcal{C}([0,1])$ :
  ...
  Soient $E$ un espace de Banach, $F$ un espace normé et $(T_n)$ une suite
  d'opérateurs linéaires continus de $E$ dans $F$
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Montrer qu'un espace de Banach de dimension infinie n'admet pas de
          base algébrique dénombrable (appliquer Baire aux sous-espaces
          engendrés par les $n$ premiers vecteurs, de dimension finie donc
          fermés).
