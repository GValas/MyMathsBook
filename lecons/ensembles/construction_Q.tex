...
\begin{tcolorbox}[thmbox]
  ...
  \]
  On note $\tfrac{a}{b}$ (ou $[a,b]$) la classe de $(a,b)$.
  $\mathbb{Q} = S/{\sim}$, muni des lois $\tfrac{a}{b}+\tfrac{c}{d}=\tfrac{ad+bc}{bd}$
  et $\tfrac{a}{b}\cdot\tfrac{c}{d}=\tfrac{ac}{bd}$, est un corps.
\end{tcolorbox}

\begin{ideepreuve}
  Trois vérifications fondent la construction.
  \emph{(i) $\sim$ est une relation d'équivalence} : la réflexivité ($ab=ba$)
  et la symétrie sont immédiates ; la transitivité repose sur l'intégrité de
  $\mathbb{Z}$. Si $ad=bc$ et $cf=de$, alors $adf=bcf=bde$, et comme $d\neq 0$
  on simplifie pour obtenir $af=be$, c'est-à-dire $(a,b)\sim(e,f)$.
  \emph{(ii) Les lois sont bien définies}, c'est-à-dire indépendantes des
  représentants choisis dans chaque classe ; les dénominateurs $bd$ sont non
  nuls car $\mathbb{Z}$ est intègre.
  \emph{(iii) $\mathbb{Z}$ se plonge} dans $\mathbb{Q}$ par le morphisme injectif
  d'anneaux $a\mapsto a/1$ ; tout élément non nul $a/b$ admet l'inverse $b/a$,
  ce qui fait de $\mathbb{Q}$ un corps.
\end{ideepreuve}
...
\begin{mdframed}[style=proofbar]
  ...
  \textit{Dénombrabilité.} On liste les fractions $a/b$, $(a,b) \in \mathbb{Z} \times \mathbb{N}^*$,
...
\begin{mdframed}[style=proofbar]
  Supposons par l'absurde qu'il existe $x \in \mathbb{Q}$ tel que $x^2 = 2$.
  Écrivons $x = p/q$ sous forme irréductible ($\pgcd(p,q) = 1$, $q > 0$).
  De $x^2 = 2$ on tire $p^2 = 2q^2$.
...
La suite de Héron $(x_n)$ définie par $x_0 = 1$ et $x_{n+1} = \tfrac{x_n}{2} + \tfrac{1}{x_n}$
est de Cauchy dans $\mathbb{Q}$ mais n'y converge pas — et tout ceci se vérifie
sans jamais quitter $\mathbb{Q}$. Un calcul direct donne
$x_{n+1}^2 - 2 = (x_n^2-2)^2/(4x_n^2)$, d'où $x_n^2 \geq 2$ puis $x_n \geq 1$
pour tout $n \geq 1$ ; il vient $0 \leq x_{n+1}^2 - 2 \leq \tfrac{1}{4}(x_n^2-2)^2$,
donc $x_n^2 \to 2$. La suite est décroissante à partir du rang $1$
(car $x_{n+1} - x_n = \tfrac{2-x_n^2}{2x_n} \leq 0$) et, pour $m \geq n \geq 1$,
\[
  0 \;\leq\; x_n - x_m \;=\; \frac{x_n^2 - x_m^2}{x_n + x_m} \;\leq\; \frac{x_n^2 - 2}{2},
\]
donc $(x_n)$ est de Cauchy. Si elle convergeait vers $\ell \in \mathbb{Q}$, on aurait
$\ell^2 = 2$, ce que le théorème précédent exclut. Cette incomplétude motive la
...
\begin{mdframed}[style=cexbar]
  ...
  Un corps ordonné n'est pas nécessairement archimédien. Mais $\mathbb{Q}$ l'est :
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Montrer que $\mathbb{Q}$ n'est pas algébriquement clos : exhiber un
          polynôme non constant de $\mathbb{Q}[X]$ sans racine dans $\mathbb{Q}$
          (on pourra considérer $X^2 - 2$).
...
\leconfooter{Euclide (irrationalité de $\sqrt{2}$, \textit{ca.} $-$300) — G.\ Cantor (dénombrabilité, 1874)}
