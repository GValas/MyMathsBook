...
\lecontitle{Construction de $\mathbb{H}$}{Théorie des ensembles — Quaternions de Hamilton}
...
\begin{mdframed}[style=proofbar]
  \textit{Inverse.} $q \cdot \bar{q} = |q|^2 \in \mathbb{R}^+$, donc
  ...
  on vérifie les $27$ triplets de base $\{1,i,j,k\}^3$.
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=proofbar]
  ...
  \[
    ...
    =
    \begin{pmatrix} a+bi & c+di \\ -\bar{c+di} & \overline{a+bi} \end{pmatrix}.
  \]
  (Matrices unitaires de $\mathrm{SU}(2)$ pour les quaternions de norme $1$.)
  \hfill$\blacksquare$
\end{mdframed}
...
\noindent{\bfseries\color{navyblue} Théorème de classification.}

\begin{mdframed}[style=exbar]
  \textbf{Théorème de Frobenius (1877).}\enspace%
  \index{Frobenius (théorème de)}
  Les seules algèbres à division de dimension finie sur $\mathbb{R}$ sont :
  $\mathbb{R}$, $\mathbb{C}$ et $\mathbb{H}$.
  ...
  normées sur $\mathbb{R}$ sont $\mathbb{R}, \mathbb{C}, \mathbb{H}, \mathbb{O}$
  (dimensions $1, 2, 4, 8$).
\end{mdframed}
...
\begin{mdframed}[style=cexbar]
  ...
  Ne pas écrire $q^2 = |q|^2$ en général : $q^2 = q\cdot q \neq q\bar{q} = |q|^2$
  (sauf si $q$ est réel pur ou scalaire).
