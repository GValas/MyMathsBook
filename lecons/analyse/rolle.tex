...
minimum. Si l'un deux est atteint en un point intérieur $c$, alors $f'(c) = 0$.
...
\begin{mdframed}[style=exbar]
  ...
  Entre deux racines consécutives de $f'$, $f$ change de monotonie.
...
\vspace{6pt}
\noindent{\bfseries\color{counterred} Contre-exemples — nécessité des hypothèses.}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Soit $f$ dérivable sur $\mathbb{R}$ avec $f(0)=0$ et $f(1)=1$.
          ...
          $f'(a)+f'(b)=2$. (\textit{Hint :} appliquer Rolle à $f(x)-x$ sur
          $[0,1/2]$ et $[1/2,1]$.)
    ...
    \item Soit $f$ continue sur $[0,1]$, $\mathcal{C}^1$ sur $]0,1[$, avec
          $f(0)=f(1)=0$ et $f'$ monotone. Montrer que $f\equiv 0$.
  \end{enumerate}
\end{mdframed}
