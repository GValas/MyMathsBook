...
\begin{mdframed}[style=proofbar]
  ...
  croissante de limite $S$ sur $[a,b]$, et la convergence de
  $\int_a^b S_N$ vers $\int_a^b S$ s'obtient en majorant chaque
  $S_N$ par $S$ (théorème de la limite monotone appliqué à l'intégrale).
  Donc
  \[
...
\begin{mdframed}[style=exbar]
  ...
  uniformément sur tout $[0,x]$ avec $|x|<1$. Une intégration terme à terme
  donne
  \[
  ...
  l'intégrale et de retrouver $\pi^2/6$ (Apéry, Beukers).

  \smallskip\noindent
  \textbf{4. Fonction $\Gamma$ et série exponentielle.}\enspace%
  $\displaystyle\int_0^{+\infty} \frac{t^{s-1}}{1+t}\,\mathrm{d}t
  = \frac{\pi}{\sin(\pi s)}$ pour $0<s<1$, via le développement
  $1/(1+t) = \sum {(-1)}^n t^n$ (Théorème~3 sur $[0,1]$, prolongement).
\end{mdframed}
...
\begin{mdframed}[style=cexbar]
  ...
  converge simplement sur $[1,+\infty[$ vers une fonction nulle hors d'un
  ensemble de mesure finie, mais $\sum\int |f_n| = \sum 1/n = +\infty$~;
