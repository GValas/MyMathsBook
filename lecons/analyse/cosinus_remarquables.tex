...
\lecontitle{Les valeurs remarquables du cosinus}{Analyse réelle — Fonctions élémentaires}
...
\begin{tcolorbox}[thmbox]
  ...
  Toute valeur du tableau se calcule par l'un des trois leviers ci-dessous.
  Plus généralement, toute valeur $\cos\!\big(\tfrac{p}{q}\pi\big)$ satisfait
  une relation polynomiale issue de $\cos(n\theta)$, que l'on résout par
  radicaux lorsque c'est possible~:
  \begin{itemize}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    \item Calculer $\cos\dfrac{7\pi}{12}$ et $\sin\dfrac{\pi}{12}$ de deux
          manières (formules d'addition, puis $\cos^2+\sin^2=1$).
