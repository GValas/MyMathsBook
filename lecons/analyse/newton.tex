...
\begin{tcolorbox}[thmbox]
  ...
  \begin{itemize}
    ...
    \item \textit{Coefficient dominant.}\enspace Le coefficient de $x^n$
          ...
          différence divisée est le coefficient de $x^k$ de l'interpolateur
    ...
    \item \textit{Interpolation d'Hermite.}\enspace\index{interpolation d'Hermite}
          Lorsque des nœuds se confondent et que $f$ est de classe
          $\mathcal{C}^m$ au voisinage du point de confluence, les
          différences divisées se prolongent par continuité : pour $m+1$
          nœuds égaux à $a$,
...
\begin{mdframed}[style=proofbar]
  ...
  $c_k$ est le coefficient de $x^k$ dans $P_k$. Montrons par récurrence
  sur $k$ la propriété suivante : \emph{pour toute famille de $k+1$ réels
  deux à deux distincts, le coefficient de $x^k$ de l'interpolateur de $f$
  en ces nœuds est la différence divisée de $f$ en ces nœuds}. Pour $k=0$,
  l'interpolateur au nœud $x_0$ est la constante $f(x_0)=f[x_0]$. Soit
  $k\geq 1$ et $x_0,\ldots,x_k$ deux à deux distincts ; soit $Q$
  (resp.\ $R$) l'interpolateur de $f$ aux nœuds $x_0,\ldots,x_{k-1}$
  (resp.\ $x_1,\ldots,x_k$). On vérifie que
  \[
  ...
  Par unicité c'est bien $P_k$ ; en identifiant les coefficients de $x^k$
  et en appliquant l'hypothèse de récurrence à $Q$ et à $R$ (deux familles
  de $k$ nœuds deux à deux distincts) :
  \[
    c_k = \frac{\bigl(\text{coeff.\ de }x^{k-1}\text{ dans }R\bigr)
                -\bigl(\text{coeff.\ de }x^{k-1}\text{ dans }Q\bigr)}{x_k-x_0}
  ...
  unicité), ni a fortiori son coefficient de $x^n$. Or ce coefficient
  vaut $f[x_0,\ldots,x_n]$ d'après l'étape~2 : la différence
  ...
  admet au moins un zéro $\xi\in I$. Or $P_n$ est de degré au plus $n$ et
  le coefficient de $x^n$ dans $P_n$ vaut $f[x_0,\ldots,x_n]$
  (étape~2), donc sa dérivée
...
\begin{mdframed}[style=exbar]
  ...
  Considérons $f(0)=1$, $f(1)=2$, $f(2)=9$, $f(3)=28$. On dresse le tableau,
  ...
  \]
  Le coefficient dominant $f[0,1,2,3]=1$ vaut bien
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Soit $f$ de classe $\mathcal{C}^n$. Montrer que
          $x\mapsto f[x_0,\ldots,x_{n-1},x]$, définie pour
          $x\notin\{x_0,\ldots,x_{n-1}\}$, se prolonge continûment en
          $x=x_{n-1}$, la limite étant la différence divisée d'Hermite
          $f[x_0,\ldots,x_{n-1},x_{n-1}]$ où le nœud doublé s'interprète
          via $f'(x_{n-1})$. \textit{Indication :} dans l'expression
          explicite de la question~5, regrouper les deux termes singuliers
          en un taux d'accroissement.
  \end{enumerate}
\end{mdframed}
