...
$f^{(n)}(\xi)/n!$ s'obtient en confrontant le coefficient dominant à la
formule d'erreur de Lagrange, via le théorème de Rolle itéré.
\end{ideepreuve}
...
\begin{mdframed}[style=proofbar]
  ...
  Supposons $f$ de classe $\mathcal{C}^{n}$ sur $I$. Posons
  $g=f-P_{n-1}$, où $P_{n-1}$ interpole $f$ aux $n$ nœuds
  $x_0,\ldots,x_{n-1}$ ; alors $g$ s'annule en ces $n$ points distincts.
  Par le théorème de Rolle appliqué entre zéros consécutifs, $g'$ admet
  $n-1$ zéros, puis $g''$ en a $n-2$, \ldots, et $g^{(n-1)}$ admet au moins
  un zéro $\xi\in I$. La formule de Newton donne
  $P_n = P_{n-1} + f[x_0,\ldots,x_n]\,\nbase{n}$, et $P_n(x_n)=f(x_n)$
  équivaut à $f(x_n)-P_{n-1}(x_n)=f[x_0,\ldots,x_n]\,\nbase{n}(x_n)$.
  Plutôt que de fixer $x_n$, exploitons directement le lien avec l'erreur
  de Lagrange : pour $x\notin\{x_0,\ldots,x_{n-1}\}$, l'interpolation aux
  $n$ nœuds donne (cf.\ leçon \emph{Lagrange})
  \[
    f(x)-P_{n-1}(x)
    = f[x_0,\ldots,x_{n-1},x]\prod_{j=0}^{n-1}(x-x_j)
    = \frac{f^{(n)}(\eta_x)}{n!}\prod_{j=0}^{n-1}(x-x_j),
  \]
  la première égalité étant la mise à jour incrémentale de Newton (le
  nœud $x$ ajouté), la seconde la formule d'erreur de Lagrange avec
  $\eta_x\in I$. En simplifiant par $\prod_{j<n}(x-x_j)\neq 0$ puis en
  posant $x=x_n$, $\xi=\eta_{x_n}$ :
  \[
