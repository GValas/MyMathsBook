...
\begin{mdframed}[style=proofbar]
  ...
  $\zeta(s)\sim 1/(s-1)$. Par ailleurs, pour $s\to+\infty$, $\zeta(s)-1=\sum_{n\geq
  2}n^{-s}\leq \sum_{n\geq 2}2^{-s}\cdot(2/n)^{s}\to 0$ (domination par
  $2^{1-s}$), d'où $\lim_{s\to+\infty}\zeta(s)=1$.

  \medskip
  \textit{Étape 5 — Produit eulérien.}
  Fixons $s>1$. Pour chaque premier $p$, $|p^{-s}|<1$ donne
  $(1-p^{-s})^{-1}=\sum_{k\geq 0}p^{-ks}$. Pour un réel $N$, le produit fini sur
...
\begin{mdframed}[style=exbar]
  ...
  En séparant termes pairs et impairs, pour $s>0$,
  \[
    \eta(s)=\sum_{n=1}^{+\infty}\frac{(-1)^{n-1}}{n^{s}}
    =\bigl(1-2^{1-s}\bigr)\,\zeta(s)\quad(s>1),
  \]
  et $\eta$ converge dès $s>0$ (série alternée). En particulier
