...
\begin{mdframed}[style=cexbar]
  ...
  fonctionnelle, pas d'une sommation de la série (cf.\ la leçon sur la somme
  $1+2+3+\cdots=-\tfrac{1}{12}$, consacrée à la régularisation zêta et à
  l'effet Casimir).
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    \item Montrer que $\zeta(s)=\dfrac{1}{s-1}+\gamma+o(1)$ quand $s\to 1^{+}$,
          où $\gamma=\lim_{N\to+\infty}\bigl(\sum_{n=1}^{N}\tfrac{1}{n}-\ln N\bigr)$
          est la constante d'Euler\index{constante d'Euler}.
          \textit{Indication :} écrire
          $\zeta(s)-\tfrac{1}{s-1}=\sum_{n\geq 1}\int_{n}^{n+1}
          \bigl(n^{-s}-t^{-s}\bigr)\,\mathrm{d}t$, majorer chaque terme par
          $s\,n^{-s-1}$ (accroissements finis) pour justifier le passage à la
          limite terme à terme sur $\,]1,2]$, puis reconnaître
          $\sum_{n\geq 1}\bigl(\tfrac{1}{n}-\ln\tfrac{n+1}{n}\bigr)=\gamma$.
