...
\begin{ideepreuve}
On coupe $[-M,M]$ en deux ; on garde une moitié dans laquelle $u_n$ tombe
pour une infinité d'indices $n$ (et non seulement de valeurs : une suite
constante n'a qu'une valeur, mais tous ses indices comptent), et on y
sélectionne un terme. En répétant, les intervalles emboîtés
...
\begin{mdframed}[style=proofbar]
  \textit{Initialisation.} Posons $I_0 = [-M,M]$ : on a $u_n \in I_0$ pour
  tout $n$, donc en particulier pour une infinité d'indices $n$. On pose
  ...
  $0 \leq j \leq k$ :
  \begin{itemize}
    \item un intervalle fermé $I_j$ de longueur $2M/2^j$ tel que
          $u_n \in I_j$ pour une infinité d'indices $n$, avec
          $I_j \subset I_{j-1}$ dès que $j \geq 1$ ;
    \item un indice $\varphi(j)$ tel que $u_{\varphi(j)} \in I_j$, avec
          $\varphi(j) > \varphi(j-1)$ dès que $j \geq 1$.
  \end{itemize}
  On divise $I_k = [a_k, b_k]$ en ses deux moitiés : l'une au moins,
  choisie pour $I_{k+1}$, vérifie encore $u_n \in I_{k+1}$ pour une
  infinité d'indices $n$. Cette infinité d'indices permet d'extraire
...
\rubriq{Remarque : l'ensemble des valeurs d'adhérence}

L'ensemble $E$ des valeurs d'adhérence d'une suite bornée est non vide
(par Bolzano--Weierstrass), borné, et fermé — cette fermeture demande un
argument d'extraction diagonale, qui fait l'objet de l'exercice 2. On
définit alors :
\[
  \limsup_{n\to\infty} u_n = \max E, \qquad
  \liminf_{n\to\infty} u_n = \min E,
\]
et la suite converge si et seulement si $\limsup u_n = \liminf u_n$.
...
\begin{mdframed}[style=exbar]
  ...
  valeur d'adhérence. Ce fait, admis ici, repose sur la densité de
  $\mathbb{Z} + 2\pi\mathbb{Z}$ dans $\mathbb{R}$ (cf.\ la leçon sur les
  ...
  chaque point de $[0,1]$ est limite d'une sous-suite. Cette densité,
  admise ici, découle de l'irrationalité de $\sqrt{2}$ (cf.\ la même
  leçon sur les sous-groupes additifs).
\end{mdframed}
...
\begin{mdframed}[style=cexbar]
  ...
  sous-suite n'est de Cauchy. La compacité séquentielle de la boule unité
  fermée caractérise la dimension finie (théorème de Riesz).
\end{mdframed}
