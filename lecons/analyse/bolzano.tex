...
Une suite bornée vit dans un espace fini : elle revient infiniment souvent au
...
On coupe $[-M,M]$ en deux ; la moitié contenant infiniment de termes est
...
\begin{mdframed}[style=proofbar]
  ...
  $(u_n)$ appartiennent à $I_0$.

  \medskip
  \textit{Induction.} Supposons construits, pour $j \leq k$ :
  \begin{itemize}
    \item un intervalle fermé $I_j \subset I_{j-1}$ de longueur $2M/2^j$
          contenant une infinité de termes de $(u_n)$ ;
    \item un indice $\varphi(j) > \varphi(j-1)$ tel que
          $u_{\varphi(j)} \in I_j$.
  \end{itemize}
  ...
  $b_k - a_k = 2M/2^k \to 0$, donc elles convergent vers le même $\ell$.
  Par encadrement :
  \[
...
\begin{mdframed}[style=exbar]
  ...
  \smallskip\noindent
  \textbf{3.}\enspace%
  $v_n = \{n\sqrt{2}\}$ est bornée dans $[0,1)$ et dense dans $[0,1]$ :
