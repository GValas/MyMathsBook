...
\begin{mdframed}[style=proofbar]
  Soit $M > 0$. Pour tout $x \in [-M, M]$, la série
  $\sum \dfrac{|x|^n}{n!}$ est dominée par
  $\sum \dfrac{M^n}{n!}$, convergente car
  $u_{n+1}/u_n = M/(n+1) \to 0$. La convergence est donc normale
  sur tout segment — d'où la convergence absolue en tout point de
  $\mathbb{R}$ — et la fonction somme est continue. La dérivabilité
...
\[
  ...
  \exp(-1) = \frac{1}{e}.
\]
Le nombre $e$\index{nombre $e$} est irrationnel (voir exercice 4), et même transcendant.
...
\begin{mdframed}[style=proofbar]
  ...
  \textit{Non-annulation.}
  Posons $p(x) = \exp(x)\exp(-x)$. D'après (1) et la dérivation d'une
  composée, $p'(x) = \exp(x)\exp(-x) - \exp(x)\exp(-x) = 0$ ; donc $p$ est
  constante, égale à $p(0) = 1$. Ainsi $\exp(x)\exp(-x) = 1$ pour tout $x$ :
  en particulier $\exp$ ne s'annule jamais, et $\exp(-x) = 1/\exp(x)$
  (point (3)). Ce point ne repose que sur $\exp' = \exp$, et non sur
  l'équation fonctionnelle établie ci-dessous.
  ...
  Puisque $\exp(-x) \neq 0$ (non-annulation), on conclut $\varphi \equiv 0$,
  i.e.\ $\exp(a+x)=\exp(a)\exp(x)$.

  \textit{(3) Signe.}
  On a déjà $\exp(x)\cdot\exp(-x) = 1$ (non-annulation), donc $\exp$ ne
  s'annule jamais et $\exp(-x) = 1/\exp(x)$. Comme $\exp(0) = 1 > 0$ et que
  $\exp$ est continue, le théorème des valeurs intermédiaires interdit tout
  changement de signe : $\exp > 0$ sur $\mathbb{R}$.

  \textit{(4) Monotonie.}
  $\exp'(x) = \exp(x) > 0$ : $\exp$ est strictement croissante.

  \textit{(5) Limites.}
  Pour $x > 0$, tous les termes de la série sont positifs, donc
  $\exp(x) \geq 1 + x \to +\infty$. Pour $x \to -\infty$ :
...
\begin{tcolorbox}[thmbox]
  ...
  \begin{enumerate}
    ...
    \]
    \item \textbf{Reste de Taylor :} pour tout $x \neq 0$
          (le cas $x = 0$ étant immédiat),
    \[
      \exp(x) = \sum_{k=0}^n \frac{x^k}{k!} + \frac{x^{n+1}}{(n+1)!}\,\exp(\xi),
      \quad \xi \text{ strictement entre } 0 \text{ et } x.
    \]
  \end{enumerate}
\end{tcolorbox}
...
\begin{mdframed}[style=proofbar]
  ...
  \textit{(3).} Formule de Taylor avec reste de Lagrange, appliquée entre
  $0$ et $x \neq 0$ : comme $\exp^{(n+1)} = \exp$ est continue sur $\mathbb{R}$,
  il existe $\xi$ strictement entre $0$ et $x$ tel que le reste vaille
  $\tfrac{\exp(\xi)}{(n+1)!}x^{n+1}$.
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=proofbar]
  \textit{Première limite.} Si $\alpha \leq 0$, alors pour $x \geq 1$ on a
  $0 \leq x^\alpha \leq 1$ et $\exp(x) \to +\infty$, donc
  $x^\alpha/\exp(x) \to 0$ trivialement. Supposons donc $\alpha > 0$ et
  posons $m = \lceil\alpha\rceil + 1$. Pour $x > 0$, la troncature donne
  $\exp(x) \geq \dfrac{x^m}{m!}$, donc
  \[
    0 \leq \frac{x^\alpha}{\exp(x)} \leq \frac{m!\,x^\alpha}{x^m}
    = \frac{m!}{x^{m-\alpha}} \xrightarrow[x\to+\infty]{} 0,
  \]
  car $m - \alpha \geq 1 > 0$.

  \textit{Limite en $-\infty$.} On l'obtient par le changement $x \mapsto -x$ :
  $|x|^\alpha\exp(x) = \dfrac{|x|^\alpha}{\exp(-x)} = \dfrac{t^\alpha}{\exp(t)} \to 0$
  en posant $t = -x \to +\infty$.

  \textit{Troisième limite.} Pour $n \in \mathbb{N}$, on applique la première
  limite à $\alpha = n+1 > 0$ : pour $x > 0$,
  \[
    \frac{\exp(x)}{x^n} = x \cdot \frac{\exp(x)}{x^{n+1}}
    = \frac{x}{\,x^{n+1}/\exp(x)\,} \xrightarrow[x\to+\infty]{} +\infty,
  \]
  puisque $x \to +\infty$ tandis que $x^{n+1}/\exp(x) \to 0$.
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{mdframed}[style=exbar]
  ...
  (la leçon sur $\ln$ le construit indépendamment par l'intégrale
  $\int_1^x \dd t/t$~; les deux constructions se rejoignent, $\ln$ et
  $\exp$ étant réciproques l'une de l'autre).
...
\begin{mdframed}[style=cexbar]
  ...
  de signe pour $x < 0$). Pour $x < 0$, la troncature à l'ordre pair
  donne une majoration, à l'ordre impair une minoration (le signe du reste
  $\frac{x^{n+1}}{(n+1)!}\exp(\xi)$ est celui de $x^{n+1}$).
\end{mdframed}
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Calculer $\displaystyle\lim_{n\to+\infty}\left(1+\frac{x}{n}\right)^n$
          pour $x\in\mathbb{R}$. (\textit{Indication :} montrer que le logarithme
          de l'expression tend vers $x$.)

    \item Résoudre l'équation différentielle $y'' - 3y' + 2y = 0$
          avec les conditions initiales $y(0) = 1$, $y'(0) = 0$.

    \item Montrer que $e \notin \mathbb{Q}$. (\textit{Indication :} supposer
          $e = p/q$, multiplier par $q!$ le développement
          $e = \sum_{k=0}^q 1/k! + \sum_{k>q} 1/k!$,
          et montrer que $q!\sum_{k>q} 1/k!$ serait à la fois un entier
          et un réel de $]0,1[$, ce qui est absurde.)
...
\leconfooter{L.\ Euler (1748) — définition par série entière ; Ch.\ Hermite (1873) — transcendance de $e$}
