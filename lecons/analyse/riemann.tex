...
\begin{tcolorbox}[thmbox]
  ...
  \[
    \forall\varepsilon > 0,\ \exists\,\sigma\ \text{subdivision tel que}\
...
\begin{tcolorbox}[thmbox]
  ...
  \begin{itemize}
    \item les fonctions \textbf{continues} (Darboux 1875)~;
...
\begin{mdframed}[style=proofbar]
  ...
  $M_i = f(x_i)$ par croissance. Donc
  \[
    S(f,\sigma) - s(f,\sigma) = \sum_i \bigl(f(x_i) - f(x_{i-1})\bigr)(x_i - x_{i-1})
    \leq |\sigma|\cdot\bigl(f(b) - f(a)\bigr).
  \]
...
\begin{mdframed}[style=exbar]
  ...
  \smallskip\noindent
  \textbf{3. Puissance par découpe géométrique.}\enspace%
  Pour $\alpha > 0$, $\int_0^1 t^\alpha\,\mathrm{d}t = \dfrac{1}{\alpha+1}$
  s'obtient par subdivision géométrique $x_k = q^k$ avec $q = a^{1/n}$,
  méthode de Fermat (avant Riemann).
  ...
  $f(t) = \lfloor 1/t \rfloor$ sur $]0,1]$, prolongée par $f(0)=0$, est
  décroissante avec une infinité dénombrable de sauts, donc Riemann-intégrable.
\end{mdframed}
...
\begin{mdframed}[style=cexbar]
  ...
  \smallskip\noindent
  \textbf{Fonctionalité non bornée.}\enspace%
  ...
  par passage à la limite $\int_\epsilon^1$.
