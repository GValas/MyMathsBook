...
\begin{tcolorbox}[thmbox]
  ...
  $f$ est Riemann-intégrable si et seulement s'il existe $I\in\mathbb{R}$ tel
...
supérieure $S(f,\sigma)$ la surestime (rectangles « par excès »).
...
\begin{tcolorbox}[thmbox]
  ...
  \begin{itemize}
    ...
    \item les fonctions \textbf{monotones} (même avec une infinité de
          discontinuités~; sur un segment, elles sont automatiquement
          bornées, encadrées par $f(a)$ et $f(b)$)~;
...
\begin{mdframed}[style=exbar]
  ...
  Riemann-intégrable (toute fonction monotone sur un segment l'est). On notera
  que $t\mapsto\lfloor 1/t\rfloor$, monotone mais non bornée, n'est définie
  que sur $]0,1]$ et ne se prolonge pas en fonction monotone (à valeurs
  finies) sur $[0,1]$~: elle n'est donc pas Riemann-intégrable sur $[0,1]$,
  l'intégrale de Riemann exigeant une fonction bornée sur tout le segment.
\end{mdframed}
