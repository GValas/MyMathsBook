...
\lecontitle{Volume de la boule en dimension \texorpdfstring{$n$}{n}}{Analyse réelle — Wallis, Fubini et géométrie en grande dimension}
...
\index{Fubini (théorème de)|seealso{Fubini--Tonelli (théorème de)}} :
\[
  V_n(R) = \int_{-R}^{R} V_{n-1}\!\bigl(\sqrt{R^2-x^2}\bigr)\dd x
          = \mu_{n-1}\int_{-R}^{R}{(R^2-x^2)}^{(n-1)/2}\dd x.
\]

\rubriq{Changement de variable et intégrales de Wallis}
On pose $x = R\cos\theta$, $\dd x = -R\sin\theta\dd\theta$
...
d'où
\[
  V_n(R) = \mu_{n-1}\,R^{n-1}\int_{\pi}^{0}(-R\sin\theta)\,\sin^{n-1}\theta\dd\theta
  = \mu_{n-1}\,R^n\int_0^{\pi}\sin^n\theta\dd\theta
  = 2\mu_{n-1}\,R^n\,W_n,
\]
en utilisant $\int_0^{\pi}\sin^n\theta\dd\theta = 2\int_0^{\pi/2}\sin^n\theta\dd\theta = 2W_n$
...
\begin{mdframed}[style=proofbar]
  \textit{Cas pair $n = 2m$.}\enspace{}La récurrence $\mu_n = \frac{2\pi}{n}\mu_{n-2}$
  donne pour $n=2k$ : $\mu_{2k} = \frac{\pi}{k}\,\mu_{2(k-1)}$.
  En itérant $m$ fois depuis $\mu_0 = 1$ :
  \[
...
\begin{array}{c|c|c}
  n & \mu_n \text{ (exact)} & \mu_n \text{ (approx.)} \\
  \hline
  1 & 2                          & 2{,}00 \\
  2 & \pi                        & 3{,}14 \\
  3 & \tfrac{4\pi}{3}            & 4{,}19 \\
  4 & \tfrac{\pi^2}{2}           & 4{,}93 \\
  5 & \tfrac{8\pi^2}{15}         & 5{,}26 \;\leftarrow\text{ maximum} \\
  6 & \tfrac{\pi^3}{6}           & 5{,}17 \\
...
\begin{tcolorbox}[thmbox]
  ...
  \[
    \mu_n \;\sim\; \frac{1}{\sqrt{\pi n}}\left(\frac{2\pi e}{n}\right)^{n/2} \xrightarrow[n\to\infty]{} 0.
  \]
\end{tcolorbox}
...
\begin{mdframed}[style=proofbar]
  ...
  En sommant les accroissements $\ln\mu_{2k} - \ln\mu_{2k-2} = \ln\frac{\pi}{k}$
  (somme télescopique) :
  \[
    \ln\mu_{2m} = \ln\mu_0 + \sum_{k=1}^{m}\ln\frac{\pi}{k} = m\ln\pi - \ln(m!)
    \;\sim\; m\ln\pi - m\ln m + m \to -\infty.
  \]
  Le cas impair se traite par le même télescopage depuis $\mu_1 = 2$. Pour obtenir
  l'équivalent précis du théorème, on applique la formule de Stirling
  $m! \sim \sqrt{2\pi m}\,(m/e)^m$ à $\mu_{2m} = \pi^m/m!$, ce qui donne
  $\mu_{2m} \sim \frac{1}{\sqrt{2\pi m}}(\pi e/m)^m$ ; en posant $n = 2m$ (donc
  $\sqrt{2\pi m} = \sqrt{\pi n}$ et $(\pi e/m)^m = (2\pi e/n)^{n/2}$) on retrouve
  $\mu_n \sim \frac{1}{\sqrt{\pi n}}(2\pi e/n)^{n/2}$, le cas impair conduisant au même équivalent.
  \hfill$\blacksquare$
\end{mdframed}

\emph{Remarque.} Les deux cas se résument en la formule unifiée
$\mu_n = \dfrac{\pi^{n/2}}{\Gamma\bigl(\frac{n}{2}+1\bigr)}$, valable pour tout $n$
(cf.\ la leçon sur la fonction Gamma, et l'exercice 3 ci-dessous) : l'équivalent
du théorème s'en déduit directement par la formule de Stirling pour $\Gamma$.
...
$\mu_n/2^n \to 0$ du cube — un aspect de la \emph{malédiction de la
dimension}\index{malédiction de la dimension}.
...
\begin{mdframed}[style=exbar]
  ...
  \smallskip\noindent
  \textbf{2. Concentration du volume près de l'équateur.}\enspace%
  \index{concentration de la mesure}%
  La méthode des tranches donne la fraction du volume de $B_n(1)$ contenue dans
  la bande centrale $\{|x_1| \leq \varepsilon\}$ :
  \[
    \frac{1}{\mu_n}\int_{-\varepsilon}^{\varepsilon}\mu_{n-1}\,{(1-t^2)}^{(n-1)/2}\dd t
    = \frac{\mu_{n-1}}{\mu_n}\int_{-\varepsilon}^{\varepsilon}{(1-t^2)}^{(n-1)/2}\dd t.
  \]
  Or la récurrence $\mu_n = 2\mu_{n-1}W_n$ et l'équivalent de Wallis
  $W_n \sim \sqrt{\pi/(2n)}$ donnent
  $\mu_{n-1}/\mu_n = \dfrac{1}{2W_n} \sim \sqrt{\dfrac{n}{2\pi}}$ :
  cette fraction tend vers $1$ pour tout
  $\varepsilon > 0$ fixé, et plus généralement dès que $\varepsilon\sqrt{n}\to+\infty$~;
  à l'échelle critique $\varepsilon = c/\sqrt{n}$, elle tend vers
  $P(|Z| \leq c) < 1$ avec $Z \sim \mathcal{N}(0,1)$ : presque tout le
  volume se concentre près de l'équateur $\{x_1 = 0\}$ — c'est le phénomène
  de \emph{concentration de la mesure}.

  \smallskip\noindent
  \textbf{3. Probabilité de tomber dans la boule} \emph{(hors programme : vecteurs
  gaussiens et loi du $\chi^2$)}\textbf{.}\enspace%
...
\begin{mdframed}[style=cexbar]
  ...
  La suite $(\mu_n)$ croît jusqu'à $n=5$, puis décroît.
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Montrer que le volume de la couronne sphérique
          $\{R_1 \leq \|x\| \leq R_2\}$ vaut $\mu_n(R_2^n - R_1^n)$ et
          calculer la fraction du volume de $B_n(1)$ contenue dans la couronne sphérique
          $\{1-\varepsilon \leq \|x\| \leq 1\}$. Que se passe-t-il quand $n\to\infty$ ?

    \item (Volume et intégrale gaussienne) Calculer $\int_{\mathbb{R}^n} e^{-\|x\|^2}\dd x$
    ...
    \item Montrer que $n = 6$ est le dernier entier tel que $\mu_n > \mu_{n-2}$,
          en utilisant la condition $\frac{2\pi}{n} > 1 \iff n < 2\pi \approx 6{,}28
          \iff n \leq 6$. (À ne pas confondre avec le maximum de la suite
          $(\mu_n)$, atteint en $n = 5$.)

    \item Soit $X \sim \mathcal{N}(0,I_n)$. Calculer $\mathbb{E}[\|X\|^2]$
          et $\mathrm{Var}(\|X\|^2)$, puis montrer par l'inégalité de
          Bienaymé--Tchebychev que pour tout $\varepsilon > 0$,
          $P\bigl(\bigl|\|X\|^2 - n\bigr| \leq \varepsilon n\bigr) \to 1$.
          En déduire que
          $P\bigl(\|X\| \in [(1-\varepsilon)\sqrt{n}, (1+\varepsilon)\sqrt{n}]\bigr) \to 1$,
          puis que $E_n = \mathbb{E}[\|X\|] \sim \sqrt{n}$ quand $n\to\infty$.
  \end{enumerate}
\end{mdframed}

\leconfooter{Wallis, Fubini — concentration de la mesure (P.\ Lévy, V.\ Milman, XX\textsuperscript{e} s.)}
