...
\begin{tcolorbox}[thmbox]
  \textbf{Théorème (Cauchy).}\enspace%
  \index{Cauchy (TAF généralisé)}
  \textit{Soient $f, g : [a,b] \to \mathbb{R}$ continues sur $[a,b]$, dérivables sur $]a,b[$.
  Alors il existe $c \in\, ]a,b[$ tel que}
  \[
...
\noindent
La preuve tient en une ligne : appliquer Rolle à
$h(x) = \bigl[g(b)-g(a)\bigr]f(x) - \bigl[f(b)-f(a)\bigr]g(x)$,
qui vérifie $h(a) = g(b)f(a) - f(b)g(a) = h(b)$.
En particulier, si $g'$ ne s'annule pas sur $]a,b[$, alors $g(b)\neq g(a)$ (par Rolle) et
...
\begin{mdframed}[style=proofbar]
  ...
  \[
    ...
    \qquad\text{i.e.}\quad f(b)-f(a) = f'(c)(b-a).
  \]
  \hfill$\blacksquare$
\end{mdframed}
...
\begin{tcolorbox}[thmbox]
  ...
  \begin{enumerate}
    \item \textbf{Constance :} $f' = 0$ sur l'intervalle $]a,b[$ $\Rightarrow$ $f$ est
          constante sur $[a,b]$.
    \item \textbf{Monotonie :} $f' \geq 0$ (resp. $> 0$) sur l'intervalle $]a,b[$
          $\Rightarrow$ $f$ est croissante (resp. strictement croissante) sur $[a,b]$.
...
\begin{mdframed}[style=proofbar]
  Pour $x < y$ dans $[a,b]$, le TAF appliqué sur $[x,y]$ donne
  $f(y)-f(x) = f'(c)(y-x) = 0$ (le cas $x = y$ étant trivial). \hfill$\blacksquare$
\end{mdframed}

\rubriq{Preuve du corollaire 2 (monotonie)}

\begin{mdframed}[style=proofbar]
  Soient $x,y \in [a,b]$ avec $x<y$. Le TAF appliqué sur $[x,y]$ fournit
  $c \in\, ]x,y[$ tel que $f(y)-f(x) = f'(c)(y-x)$. Comme $y-x>0$, si $f'\geq 0$
  ce produit est positif ou nul, donc $f(y)\geq f(x)$ : $f$ est croissante ;
  si $f'>0$, il est strictement positif, donc $f(y)>f(x)$ : $f$ est strictement
...
Supposons $f,g$ continues sur $[a,a+\delta[$ et dérivables sur $]a,a+\delta[$,
avec $f(a)=g(a)=0$, $g'$ ne s'annulant pas sur $]a,a+\delta[$,
et $\lim_{x\to a^+} f'(x)/g'(x) = \ell$. Alors par le TAF de Cauchy appliqué sur $[a, a+h]$,
il existe $c_h \in\, ]a, a+h[$ tel que
\[
  \frac{f(a+h)}{g(a+h)} = \frac{f(a+h)-f(a)}{g(a+h)-g(a)} = \frac{f'(c_h)}{g'(c_h)} \xrightarrow[h\to 0]{} \ell.
\]
La continuité de $f$ et $g$ en $a$ assure $f(a)=g(a)=0$ dans la première égalité ;
de plus, comme $c_h \in\, ]a, a+h[$, on a $c_h \to a$ quand $h \to 0$, d'où le passage
à la limite $f'(c_h)/g'(c_h) \to \ell$.
...
\begin{mdframed}[style=exbar]
  ...
  Si $|f'| \leq k < 1$ sur un intervalle \emph{fermé} stable, alors $f$ est une
  ...
  la fermeture garantit l'existence du point fixe dans l'intervalle).
\end{mdframed}
...
\begin{mdframed}[style=cexbar]
  ...
  La version vectorielle correcte est l'inégalité : $\|f(b)-f(a)\| \leq \sup_{t\in[a,b]}\|f'(t)\|\cdot(b-a)$.
  ...
  \smallskip\noindent
  \textbf{$f'>0$ est suffisant mais non nécessaire à la stricte croissance.}\enspace%
  $f(x)=x^3$ est strictement croissante sur $\mathbb{R}$, alors que $f'(0)=0$ :
  la réciproque du corollaire de monotonie (cas strict) est fausse.

  \smallskip\noindent
  \textbf{La continuité sur l'intervalle fermé est nécessaire.}\enspace%
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    ...
    \item Soit $f$ dérivable sur $\mathbb{R}$ avec $f(0)=1$ et $f'(x) \geq f(x)$
          ...
          (\textit{Indication :} étudier $g(x)=f(x)e^{-x}$.)

    \item Montrer que si $f$ est continue sur $[a,b[$, dérivable sur $]a,b[$,
          et si $f'$ admet une
    ...
    \item Soit $f : [0,1]\to\mathbb{R}$ dérivable avec $f(0)=0$ et $|f'(x)|\leq |f(x)|$
          ...
          (\textit{Indication :} majorer $\max_{[0,t]}|f|$ à l'aide du TAF, puis itérer.)
  \end{enumerate}
\end{mdframed}
