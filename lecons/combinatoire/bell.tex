...
\begin{tcolorbox}[thmbox]
  ...
  Une \emph{partition} de l'ensemble $[n] = \{1,2,\ldots,n\}$
...
\begin{mdframed}[style=exbar]
  ...
  $\bigl\{\{1,2,3\}\bigr\}$,
  $\bigl\{\{1\},\{2,3\}\bigr\}$, $\bigl\{\{2\},\{1,3\}\bigr\}$,
  $\bigl\{\{3\},\{1,2\}\bigr\}$,
  $\bigl\{\{1\},\{2\},\{3\}\bigr\}$.
...
\begin{mdframed}[style=exbar]
  ...
  \end{itemize}
  Le dernier terme de chaque ligne est $B_{n+1}$ : cette propriété est une
  conséquence directe de la récurrence de Bell ci-dessus — on vérifie par
  récurrence sur $k$ (via la formule de Pascal) que
  $a_{n,k} = \sum_{i=0}^{k}\binom{k}{i}B_{n-k+i}$, et le cas $k=n$ redonne
  exactement $B_{n+1} = \sum_{i=0}^{n}\binom{n}{i}B_i$.
...
\begin{tcolorbox}[thmbox]
  ...
  \medskip
  La somme $\sum_{j=0}^{k}{(-1)}^{k-j}\binom{k}{j}j^n$ compte, par
  inclusion-exclusion, les surjections de $[n]$ sur $[k]$ : elle est donc
  nulle dès que $k > n$, et la formule reste valable pour tout $k \geq 0$.
\end{tcolorbox}
...
\begin{tcolorbox}[thmbox]
  ...
  \index{formule de Dobinski@formule de Dobi\'nski}
  \[
...
\begin{mdframed}[style=proofbar]
  ...
  la formule d'inclusion-exclusion reste valable pour $k>n$, où elle donne
  $S(n,k)=0$ (elle compte les surjections de $[n]$ sur $[k]$, cf.\ la remarque
  du paragraphe~3), donc les termes ajoutés sont nuls. Dans la double somme ainsi
  obtenue, seuls les indices $0\leq j\leq k$ contribuent.
  Intervertissons alors les deux sommations : la série double converge
  absolument, car la somme des valeurs absolues se factorise en
  \[
    \sum_{j\geq 0}\sum_{k\geq j}\frac{j^n}{j!\,(k-j)!}
      = \Bigl(\sum_{j\geq 0}\frac{j^n}{j!}\Bigr)
        \Bigl(\sum_{\ell\geq 0}\frac{1}{\ell!}\Bigr)
      = e\sum_{j\geq 0}\frac{j^n}{j!} < +\infty,
  \]
  ce qui autorise l'interversion. En sommant d'abord sur $k\geq j$ :
  \[
...
\begin{mdframed}[style=cexbar]
  ...
  On a déjà $B_n \geq S(n,2) = 2^{n-1}-1$ pour $n\geq 1$ (voir
  l'exercice~2), donc une croissance au moins exponentielle, mais la
  croissance réelle est bien plus rapide :
  \[
    \ln B_n = n\bigl(\ln n - \ln\ln n - 1\bigr) + o(n)
  \]
  (asymptotique de Moser--Wyman / de Bruijn). On ne confondra donc pas $B_n$
  avec $n!$ ni avec $2^n$~: pour $n$ assez grand, $2^n \ll B_n \ll n!$.
  ...
  $k$ cycles, et non les partitions. Ils vérifient une récurrence analogue,
  mais avec le coefficient $n-1$ au lieu de $k$~:
  $\begin{bmatrix}n\\k\end{bmatrix}
   = (n-1)\begin{bmatrix}n-1\\k\end{bmatrix}
   + \begin{bmatrix}n-1\\k-1\end{bmatrix}$
  (la version \emph{signée} $s(n,k)$ introduit de plus un signe).
...
\begin{mdframed}[style=exobar]
  \begin{enumerate}
    \item En utilisant la récurrence $B_{n+1} = \sum_{k=0}^{n}\binom{n}{k}B_k$,
          vérifier que $B_6 = 203$ à partir des valeurs précédentes.
    ...
    \item Montrer que $B_n$ est impair si et seulement si $n\equiv 0$ ou
          ...
          (\textit{Indication :} établir d'abord
          $B_{n+2} \equiv B_{n+1} + B_n \pmod{2}$ en considérant l'involution
          qui échange les éléments $n+1$ et $n+2$ dans une partition de
          $[n+2]$ : ses points fixes sont les partitions où $n+1$ et $n+2$
          sont dans le même bloc, ou tous deux des singletons.)
...
\leconfooter{%
  G.\ Dobi\'nski (1877) · E.\ T.\ Bell, \textit{Am.\ Math.\ Monthly} (1934)%
}
